...
The next step is to activate theme. From your terminal, add the **ananke** theme to the `config.toml` file.
...
You can now your new project running locally with the following command:
...
By default, Hugo doesn't generate a `package.json` file, so you'd have to add a `package.json` file to the `my-hugo-project` directory.

Next, run the following command to add the `hugo-cli` package.
...
For the build property that you configured, add a `now-build` script in the `package.json` file to specify what command Now will run to build the app in the cloud.
...
Finally, before deploying, ensure the `draft` value in your `my-first-post.md` file is set to `false`. If it remains as `true`, the content of your posts won't show up in production.
...
When your app is deployed, you will receive a deployment URL like the following: [https://my-hugo-project-h8wsh8ii0.now.sh/](https://my-hugo-project-h8wsh8ii0.now.sh/)
