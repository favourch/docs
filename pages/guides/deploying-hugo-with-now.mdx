...
In this guide, we will set up a Hugo website and deploy it with ZEIT Now.
...
Once you are done running the command, edit the `my-first-post.md` file and add the following content:
...
The `now.json` file allows your deployment achieve a few things:
...
Ensure that you run the following command on the `build.sh` file to make it executable.

<TerminalInput>chmod +x build.sh</TerminalInput>
<Caption>
  A command to grant appropriate permissions to make <InlineCode>build.sh</InlineCode> executable
</Caption>

And with this configuration set up, you can now deploy your Hugo project.

<Note>
  Finally, before deploying, ensure the <InlineCode>draft</InlineCode> value in
  your <InlineCode>my-first-post.md</InlineCode> file is set to{' '}
...
When your app is deployed, you will receive a deployment URL like the following: [https://my-hugo-project-kj4fu0pcq.now.sh/](https://my-hugo-project-kj4fu0pcq.now.sh/)

## Bonus: Cache Your Hugo Assets

Caching makes your site fast and reduces bandwith consumption for your users. You can configure Now's routes within your `now.json` configuration file to set up caching headers.
...
```json
{
  ...
  "routes": [
    {"src": "^/dist/js/(.*)", "headers": {"cache-control": "public,max-age=31536000,immutable"}, "dest": "/js/$1" },
    {"src": "^/dist/css/(.*)", "headers": {"cache-control": "public,max-age=31536000,immutable"}, "dest": "/css/$1"},
    {"src": "^/images/(.*)", "headers": {"cache-control": "max-age=31536000,immutable"}, "dest": "/images/$1"},
    {"src": "^/(.*).html", "headers": {"cache-control": "public,max-age=0,must-revalidate"}, "dest": "/index.html"}
  ]
}
```
...
<Card
  title="Deploying Gatsby with Now"
